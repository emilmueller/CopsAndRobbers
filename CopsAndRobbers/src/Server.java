...
public class Server {
	...
	class CopsProgram extends Program {
		CopsProgram(String cmd) {
			super(cmd);
			String copsJar = cmd.substring(cmd.lastIndexOf('/', cmd.length())+1);
			System.out.println("Cops: "+copsJar);
		}
	...
	class MouseXProgram extends Program {
		MouseXProgram(String cmd) {
			super(cmd);
			String mouseJar = cmd.substring(cmd.lastIndexOf('/', cmd.length())+1);
			System.out.println("Mouse: "+mouseJar);
		}
		void feedOwnPos() throws IOException {
			feedMouseXPos();
		}
